...
# SYNC Function Propertly
    if [[ -z "$1" ]]; then
        ...
        if [[ -f "$PKG_install_dir/config/src/config.sh" ]]; then
            source $PKG_install_dir/config/src/config.sh
        else
            echo "error: The script used to manage the configuration file is missing. Build the config file manually."
        fi
...
### sync dirs
        ...
        sync -f
        sync -d
### create a template for the config file
